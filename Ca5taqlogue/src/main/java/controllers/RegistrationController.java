...
public class RegistrationController {
	...
	// These four variables to login and view the gnomes, the rest can be added
	// in the account page
	...
	public String register() {
		...
		if (!name.isEmpty() && !email.isEmpty() && !password.isEmpty()) {
			if (registrationService.validRegistration(email)) {
				currentUser.setCustomer(registrationService.registerUser(name, lastName, email, password));
				return "Gnomepage2";
			} else {
				name = "";
				lastName = "";
